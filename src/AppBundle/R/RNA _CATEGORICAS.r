#Red neurona sin PCA con variables categoricas
...
#############################
# PRUEBA CON variables cuantitativas y cualitativas
##############################
...
########################
P3<-data[indice,!(names(data) %in% c("cant_mat","cal_1"))]
P3

## Creamos el objeto red neuronal feedforward
############################################
net3 <- newff(n.neurons=c(ncol(P3),5,4,2), learning.rate.global=1e-2, momentum.global=0.5,
error.criterium="LMS", Stao=NA, hidden.layer="sigmoid",
output.layer="sigmoid", method="ADAPTgdwm")
#############################
##Se entrena la red
#######################
result3 <- train(net3,P3 , target, error.criterium="LMS", report=TRUE, show.step=10000, n.shows=39 )
######################
# se grafica la red entrenada con los datos de entrenamiento
####
s <- sim(result3$net, P3)
